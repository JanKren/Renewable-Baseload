# -*- coding: utf-8 -*-
"""
Download Solar Energy Data from ENTSO-E (2015-2024)

Downloads solar generation data for all European countries from 2015 to 2024.
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Data", "Solar")

START_YEAR = 2015
END_YEAR = 2024

# Main countries (same as wind analysis)
COUNTRIES = {
    'DE': 'Germany',
    'ES': 'Spain',
    'FR': 'France',
...
def download_solar_data(client, country_code, start_str, end_str):
    """Download solar generation data for a specific period."""
    ...
    try:
        ...
        # Extract solar columns from MultiIndex columns
        solar_data = {}
        ...
        for col in df.columns:
            ...
            # Look for solar columns
            if 'solar' in col_name and 'actual' in col_type and 'consumption' not in col_type:
                solar_data['Solar'] = df[col]

        if not solar_data:
            return None

        result = pd.DataFrame(solar_data)
        return result
...
def download_country(client, code, name):
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        for q_num, (start_str, end_str) in enumerate(get_quarters(year), 1):
            ...
            df = download_solar_data(client, code, start_str, end_str)
    ...
    if all_data:
        ...
        output_file = os.path.join(OUTPUT_DIR, f"{code}_solar_2015_2024.csv")
        combined.to_csv(output_file)
        print(f"  -> Saved: {len(combined)} rows to {output_file}")
        return True
    else:
        print(f"  -> No solar data for {code}")
        return False
...
def main():
    ...
    print("=" * 60)
    print(f"ENTSO-E Solar Data Download: {START_YEAR}-{END_YEAR}")
